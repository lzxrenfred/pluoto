...
import { ACCESSORIES, applyDecorationPreset, BODY_COLORS, DECORATION_PRESETS, GROUNDS, HOMES, HOUSE_COLORS, OUTFITS, SPECIES, normalizedPerson } from "@/lib/customization";
import { PILL_GROUPS } from "@/lib/demo";
import type { DecorationPreset, Person } from "@/lib/types";
  ...
  return <div className="onboarding-shell">
    ...
    <main className={`onboarding-main step-${step}`}>
      ...
      <section className="onboarding-panel">
        ...
        {step === NAME && <><p className="eyebrow">YOUR NAME</p><h1>What should we call you?</h1><label className="field-label">Nickname<input autoFocus maxLength={18} value={draft.nickname} onChange={event => update({ nickname: event.target.value })} placeholder="e.g. Ren"/></label><p className="quiet-copy">This is what people in your Plane will see.</p></>}
        {step === CHARACTER && <><p className="eyebrow">YOUR CHARACTER</p><h1>Make a little you.</h1><h3>Animal</h3><div className="option-grid species-options">{SPECIES.map(item => <button key={item} className={draft.species === item ? "selected" : ""} onClick={() => update({ species: item })}>{item}</button>)}</div><h3>Fur or body</h3><div className="swatches">{BODY_COLORS.map(item => <button key={item} aria-label={item} className={draft.color === item ? "selected" : ""} style={{ background: item }} onClick={() => update({ color: item })}/>)}</div><h3>Top</h3><div className="option-grid">{OUTFITS.map(item => <button key={item} className={draft.outfit === item ? "selected" : ""} onClick={() => update({ outfit: item })}>{item === "none" ? "No top" : "Soft tee"}</button>)}</div><h3>Accessory</h3><div className="option-grid">{ACCESSORIES.map(item => <button key={item} className={draft.accessory === item ? "selected" : ""} onClick={() => update({ accessory: item })}>{item}</button>)}</div></>}
        {step === PILLS && <><p className="eyebrow">IDENTITY PILLS</p><h1>A few things about you.</h1><p>Choose as many as feel useful, or skip for now.</p>{Object.entries(PILL_GROUPS).map(([group, items]) => <details key={group} open={group === "Personality" || group === "Interests"}><summary>{group}<span>{items.filter(item => draft.pills.includes(item)).length || ""}</span></summary><div className="pills selectable">{items.map(pill => <button key={pill} className={draft.pills.includes(pill) ? "selected" : ""} onClick={() => togglePill(pill)}>{draft.pills.includes(pill) && <Check size={12}/>} {pill}</button>)}</div></details>)}<button className="text-button" onClick={() => go(LAND)}>Skip for now</button></>}
        {step === LAND && <><p className="eyebrow">YOUR 5 × 5 LAND</p><h1>Make your corner feel like home.</h1><h3>Ground</h3><div className="option-grid">{GROUNDS.map(item => <button key={item} className={draft.ground === item ? "selected" : ""} onClick={() => update({ ground: item })}>{item === "earth" ? "warm earth" : item}</button>)}</div><h3>House</h3><div className="option-grid">{HOMES.map(item => <button key={item} className={draft.home === item ? "selected" : ""} onClick={() => update({ home: item })}>{item}</button>)}</div><h3>House colour</h3><div className="option-grid">{HOUSE_COLORS.map(item => <button key={item} className={draft.houseColor === item ? "selected" : ""} onClick={() => update({ houseColor: item })}>{item}</button>)}</div><h3>Decoration set</h3><div className="option-grid">{DECORATION_PRESETS.map((item: DecorationPreset) => <button key={item} className={draft.decorationPreset === item ? "selected" : ""} onClick={() => setDraft(value => applyDecorationPreset(value, item))}>{item}</button>)}</div><p className="quiet-copy">Every set is grid-safe and leaves open ground for movement.</p></>}
        {error && <div className="form-error" role="alert">{error}</div>}
      </section>
    </main>
    {(step === AUTH || step === NAME || step === CHARACTER || step === PILLS || step === LAND || step === RESET_PASSWORD) && <footer className="onboarding-footer"><button className="primary" type={step === AUTH ? "submit" : "button"} form={step === AUTH ? "pluoto-auth-form" : undefined} disabled={step === AUTH ? busy || !/^\S+@\S+\.\S+$/.test(email) || password.length < MIN_PASSWORD_LENGTH || (intent === "create" && password !== confirmPassword) : disabled} onClick={step === AUTH ? undefined : () => void primary()}>{busy ? "One moment…" : step === AUTH ? intent === "signin" ? "Sign in" : "Create account" : step === LAND && editing ? "Save changes" : step === LAND ? "Enter my Plane" : step === RESET_PASSWORD ? "Save new password" : "Continue"}</button></footer>}
  </div>;
}
