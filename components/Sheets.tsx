...
export function CustomizeFlow({ owner, onSave, onClose }: CustomizeProps) {
  ...
  return <div className="full-flow">
    ...
    <main>
      {step === 0 && <div className="flow-step intro-step"><div className="mini-world"><Character species={draft.species} color={draft.color} accent={draft.accent} accessory={draft.accessory} size={116}/></div><p className="eyebrow brand-eyebrow">WELCOME TO <BrandWordmark/></p><h1>My people,<br/>around me.</h1><p>Make a little you, then build your corner of the sky.</p><label>Your nickname<input value={draft.nickname} maxLength={18} onChange={(e) => setDraft({ ...draft, nickname: e.target.value })}/></label></div>}
